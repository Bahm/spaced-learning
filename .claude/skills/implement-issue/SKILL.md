...
6. **Best-practices audit** — routinely verify the project follows Claude Code best practices. Check each item and fix any drift:
   ...
   - [ ] Path-scoped rules exist in `.claude/rules/` for each major code area (`src/domain/`, `src/db/`, `tests/e2e/`, `.claude/scripts/`, `.github/`) with valid YAML frontmatter and `paths:` keys
